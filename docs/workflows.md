...
2. **检查 `resources/`**：如果需求涉及外部服务（数据库、API、第三方接口），先读 `resources/` 下的相关文档获取连接信息和接口规范
3. **判断需求复杂度**：
...
2. **立即写入**：
   ...
   - 外部服务信息缺失（数据库、API 地址等）→ 写入 `resources/` 下对应文件
...
> `ai-memory/structure/modules/*.md` 不是"可选文档"。满足以下任一条件，必须新增或更新对应模块文档，且必须在本轮变更完成的同时完成，不得延迟。

**触发条件（任一命中即触发）**：

1. 新增业务模块（新子域、新服务边界）
2. 模块职责或边界变化（拆分、合并、归属调整）
3. 跨模块数据流或调用链变化（输入输出、事件流、关键依赖）
4. 模块关键约束变化（幂等、一致性、缓存、降级、权限、SLA）
5. 同一模块在最近 14 天内被"临时重新探索"≥2 次（说明文档缺失或严重过时）

**执行要求**：

- 代码变更和模块文档更新必须同步完成，未更新模块文档前不得标记该子任务为完成。
- 若仅有局部变更，至少更新模块文档的"最近更新"和"最近变更影响"两个段落。
- 模块文档最小模板见本 skill 的 `docs/reference.md`。
