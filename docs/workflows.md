...
> 来自 OpenAI 实践：当 Agent 遭遇困难，正确响应不是"努力重试"，而是诊断**环境缺失了什么**，并立即修补。
...
1. **诊断缺失**：问自己"如果这个信息已经写在某个文档里，我就不会卡住了——那应该写在哪里？"
2. **立即写入**：
   - 规则/约定缺失 → 写入 `DECISIONS.md` 或 `TWEAKS.md`
   - 接口/模块文档缺失 → 在 `ai-memory/structure/modules/` 下创建补充文档
   - 架构信息缺失 → 补充 `ARCHITECTURE.md`
3. **再继续实现**：环境修补后，立即继续任务

**不确定但非阻塞的点**：用 `//todo (AI) <原因> -- YYYY-MM-DD` 标记，任务完成后统一处理

**需要用户澄清的需求**：暂停实现，先提问，获得答案后把答案编码进文档（不要留在对话里）
