...
description: >
  ...
  项目涉及 ai-memory/ 目录、需要了解 CURRENT_TASK.md / TWEAKS.md / CHANGELOG.md 等记忆文件、
  或用户提到需求实现、子需求拆分、跨会话开发续接。
  即使用户只是说"帮我做XX功能"但项目根目录存在 ai-memory/ 目录，也应先加载此 skill
...
如果 `ai-memory/` 目录不存在，按顺序创建：

1. **创建目录结构**（见第一节的树形结构）
2. **创建最小可用文件**：
   - `context/CURRENT_TASK.md` — 初始状态（见模板）
   - `context/TWEAKS.md` — 空文件，仅含标题和格式说明
   - `context/DECISIONS.md` — 空文件，仅含标题
   - `changed/CHANGELOG.md` — 空文件，仅含标题
   - `structure/ARCHITECTURE.md` — 从项目实际结构生成
   - `structure/TECH_STACK.md` — 从项目依赖生成
   - `BOOTSTRAP.md` — 指向本 skill 的简短入口文件
3. **初始化 BOOTSTRAP.md**（项目根目录）：
   ```markdown
   # AI 开发工作流
...
3. **每完成一个子需求后**，按顺序执行：
   ...
   - 迁移需求文档至 `demand/done/`
   - 在 `CHANGELOG.md` 追加一行摘要（`- YYYY-MM-DD: {摘要} → [详情](changed/records/...)`）
...
1. **创建需求文档** — 在 `demand/` 下创建 `{YYYY-MM-DD}_{简短描述}.md`，内容包含：
   - 需求背景与目标
   - 功能点拆解（子需求列表）
   - 涉及模块/文件（初步判断）
   - 验收标准
...
当已归档的需求（位于 `demand/done/`）需要变更时：

1. **定位原需求** — 在 `demand/done/` 找到对应需求文档，同时在 `changed/records/` 查找当时的变更记录
2. **创建变更需求文档** — 在 `demand/` 下创建 `{YYYY-MM-DD}_{原需求名}_变更.md`，内容包含：
   - 关联原需求：`见 demand/done/{原需求文件}`
   - 变更内容（新增/修改/删除哪些功能点）
   - 影响范围分析（涉及哪些模块、可能牵连的功能）
3. **更新 CURRENT_TASK.md** — 同 4.3 步骤 2
4. **完成后**：
   - 将原需求文档从 `demand/done/` 移到 `demand/done/archive/`（如果变更较大，原文档已过时）
   - 或将变更说明追加到原需求文档末尾，保留在 `demand/done/`（如果变更较小）
   - 变更需求文档移至 `demand/done/`
   - 在 `changed/records/` 追加变更记录，关联原记录
...
用户对 AI 的理解偏差进行纠正后，**必须立即**追加到 `ai-memory/context/TWEAKS.md`。
...
- **冷启动时读取 TWEAKS.md，其中的「规则」字段视为硬约束**
...
1. **不要重复探索**：已经探索过的目录结构、模块关系，应写入 `ARCHITECTURE.md`
2. **不要重复犯错**：每次被纠正，立即写 TWEAKS.md
3. **定期归档**：`changed/records/` 中超过 3 个月且已上线的需求记录可归档至 `changed/records/archive/`
...
- [ ] **需求文档** 是否已归档？已完成的需求从 `demand/` 移到 `demand/done/`
